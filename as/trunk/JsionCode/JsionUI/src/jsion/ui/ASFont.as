...
package jsion.ui
{
	...
	public class ASFont implements IDispose
	{
		 ...
		 */
		public function ASFont(nameOrTextFormat:* = "Tahoma", size:Number = 14, bold:Boolean = false, italic:Boolean = false, underline:Boolean = false, embedFontsOrAdvancedPros:* = null)
		{
		...
		{
			return UIUtil.computeStringSizeWithFont(this, text, includeGutters);
		}
		...
		public function makeFullFeatured():ASFont
		{
			if(!isFullFeatured())
			{
				return takeover(DefaultRes.DEFAULT_FONT);
			}
			else
			{
				return this;
		...
		{
			if(this == DefaultRes.DEFAULT_FONT) return;
