...
package jsion.chat
{
	...
	import jsion.chat.plugins.IRTFPlugin;
	...
	public class RichTextField extends Sprite
	{
		...
		private function getSpriteFromObject(obj:Object):DisplayObject
		{
			if (obj is String) 
			{
				if(String(obj).toLowerCase().indexOf("event:") == 0)
				{
		...
		protected function createLink(eventData:Object):TextField
		{
			var tf:TextField = new TextField();
			
			tf.textColor = 0x3366CC;
			tf.type = TextFieldType.DYNAMIC;
			tf.autoSize = TextFieldAutoSize.LEFT;
			tf.selectable = false;
			tf.defaultTextFormat = new TextFormat(null, 14, null, null, null, true);
			tf.htmlText = "<a href='" + String(eventData) + "'>新建链接</a>";
		...
		public function importXML(data:XML):void
		{
			...
			for (var i:int = 0; i < data.sprites.sprite.length(); i++)
			{
				...
				if (html && contentLength > 0 && index > 0) sprite.index = index + 1;
				else sprite.index = index;
				sprites.push(sprite);
			}			
