    ...
    public class DbHelper
    {
        ...
        public static string ConnectionString
        {
            get
            {
                ...
                {
                    _connectionstring = BaseConfigs.GetDBConnectString;
                }
                ...
                if (_provider == null)
                {
                    lock (lockHelper)
                    {
                        if (_provider == null)
                        {
                            ...
                            {
                                // MS是映射
                                _provider = (IDbProvider)Activator.CreateInstance(Type.GetType(string.Format("Sjs.Data.{0}Provider,Sjs.Data.{0}", BaseConfigs.GetDbType), true, true));
                            }// endtry
                            catch
                            {
        ...
        {
            DbParameter[] cloneParameters = new DbParameter[originalParameters.Length];

            for (int i = 0, j = originalParameters.Length; i < j; i++)
            {
                cloneParameters[i] = (DbParameter)((ICloneable)originalParameters[i]).Clone();
            }
        ...
        /// <summary>
        /// 执行指定查询字符串类型的DbCommand.
        /// </summary>
        /// <remarks>
        /// 示例:  
        ///  int result = ExecuteNonQuery("SELECT * FROM [table1]");
        /// </remarks>
        /// <param name="commandText">存储过程名称或SQL语句</param>
        /// <returns>返回命令影响的行数</returns>
        public static int ExecuteNonQuery(string commandText)
        ...
        public static int ExecuteNonQuery(DbTransaction transaction, string spName, params object[] parameterValues)
        {
            ...
            {
                //从缓存中加载存储过程参数,如果缓存中不存在则从数据库中检索参数信息并加载到缓存中.
                DbParameter[] commandParamenters = GetSpParameterSet(transaction.Connection, spName);

                //给存储过程参数赋值
                AssignParameterValues(commandParamenters, parameterValues);

                //调用重载方法
                return ExecuteNonQuery(transaction, CommandType.StoredProcedure, spName, commandParamenters);
        ...
        public static void ExecuteCommandWithSplitter(string commandText, string splitter)
        {
            ...
            do
            {
                int lastPos = commandText.IndexOf(splitter, starPos);
                int len = (lastPos > starPos ? lastPos : commandText.Length) - starPos;
                string query = commandText.Substring(starPos, len);
                ...
                {
                    starPos = lastPos + splitter.Length;
                }

            } while (starPos < commandText.Length);
        }
        ...
        public static DataSet ExecuteDataset(DbConnection connection, CommandType commandType, string commandText, params DbParameter[] commandParameters)
        {
            ...
            // 预处理
            DbCommand cmd = Factory.CreateCommand();
            bool mustColseConnection = false;
            PrepareCommand(cmd, connection, (DbTransaction)null, commandType, commandText, commandParameters, out mustColseConnection);
        ...
        /// <param name="includeReturnValueParameter">是否包含返回值参数</param>
        /// <returns>返回DbParameter参数数组</returns>
        public static DbParameter[] GetSpParameterSet(string spName, bool includeReturnValuesParameter)
        {
            if (string.IsNullOrEmpty(ConnectionString))
            {
                throw new ArgumentNullException("ConnectionString");
            ...
            {
                connection.ConnectionString = ConnectionString;
                return GetSpParameterSet(connection, spName, includeReturnValuesParameter);
            }
        ...
        /// 生成输入参数
        /// </summary>
        /// <param name="ParamName">参数名,如:@username</param>
        ...
        /// <summary>
        /// 执行ExecuteScalar,将结果以字符串类型输出(无参数)
        /// </summary>
        /// <param name="commandType">CommandType类型:文本/存储过程</param>
        /// <param name="commandText">SQL查询语句</param>
        /// <returns>执行ExecuteScalar,将结果以字符串类型输出</returns>
        public static string ExecuteScalarToStr(CommandType commandType, string commandText)
        {
            object ec = ExecuteScalar(commandType, commandText);
            if (ec == null)
        ...
        /// <summary>
        /// 执行ExecuteScalar,将结果以字符串类型输出(有参数)
        /// </summary>
        /// <param name="commandType">CommandType类型:文本/存储过程</param>
        /// <param name="commandText">SQL查询语句</param>
        ...
        public static int ExecuteScalarToInt(CommandType commandType, string commandText, DbParameter[] commandParameters)
        {
            object ec = ExecuteScalar(commandType, commandText, commandParameters);
            if (ec == null)
            {
