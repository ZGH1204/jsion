package jcomponent.org.coms.sliders
{
	...
	public class BasicSliderUI extends BasicComponentUI
	{
		...
		protected function paintSlider(component:Component):void
		{
			...
			if(slider.dir == AbstractSlider.HORIZONTAL)
			{
				JUtil.layoutPosition(viewRect, UIConstants.LEFT, UIConstants.MIDDLE, slider.hThumbHGap, slider.hThumbVGap, tmpRect);
				
				thumb.rang = viewRect.width - thumb.width;
			}
			else
			{
				JUtil.layoutPosition(viewRect, UIConstants.CENTER, UIConstants.TOP, slider.vThumbHGap, slider.vThumbVGap, tmpRect);
		...
		protected function locateBackground(component:Component):void
		{
			...
			if(bg && bg.getDisplay(component))
			{
				...
				if(dis && dis.width > 1 && dis.height > 1)
				{
					...
					if(slider.dir == AbstractSlider.HORIZONTAL)
					{
						JUtil.layoutPosition(viewRect, UIConstants.LEFT, UIConstants.MIDDLE, 0, 0, bgRect);
					}
					else
					{
						JUtil.layoutPosition(viewRect, UIConstants.CENTER, UIConstants.TOP, 0, 0, bgRect);
					}
